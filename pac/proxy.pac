// 1. use the pac rules of unblock-youku to unblock video sites.
// 2. use local shadowsocks proxy to unblock NetEase music.
// https://github.com/kuyur/proxy.pac

var full_proxy_domains = {
  'music.163.com': 1,
  'music.126.net': 1
};

var local_proxy = 'SOCKS5 127.0.0.1:11080; SOCKS 127.0.0.1:11080; DIRECT;';
var unblock_youku_proxy = "PROXY proxy.uku.im:443; DIRECT;";
var direct = 'DIRECT;';

function FindProxyForURL(url, host) {
  var lastPos;
  do {
    if (full_proxy_domains.hasOwnProperty(host)) {
      return local_proxy;
    }
  ...
  var prefix = url.slice(0, 6);
  return "http:/" === prefix ? _find_proxy(_http_map, host, url, 7) : "https:" === prefix ? _find_proxy(_https_map, host, url, 8) : direct;
}
...
function _check_patterns(map, host, url, len) {
  return map.hasOwnProperty(host) && _check_regex_list(map[host], url.slice(len + host.length)) ? !0 : !!_check_regex_list(map.any, url.slice(len));
}

function _find_proxy(map, host, url, len) {
  return _check_patterns(map.white, host, url, len) ? direct : _check_patterns(map.proxy, host, url, len) ? unblock_youku_proxy : direct;
}
